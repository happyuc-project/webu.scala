...
import com.micronautics.webuj._
import com.micronautics.webuj.WebuJScala._
import org.webuj.protocol.core.DefaultBlockParameterName._
import org.webuj.protocol.core.methods.{request, response}

/** WebuJ's functional-reactive nature makes it easy to set up observers that notify subscribers of events taking place on the blockchain.
  * The functional-reactive programming style works really well with Scala.
  *
  * Other transaction and block replay [[org.webuj.utils.Observables]] are described in [[https://docs.webuj.io/filters.html Filters and Events]]. */
class DemoObservables(demo: Demo) {
  import demo._

  //  Display the first 2 new blocks as they are added to the blockchain:
  observe(2)(webuj.blockObservable(false)) { hucBlock =>
    println(format(hucBlock))
  }

  // Display only the first 2 new transactions as they are added to the blockchain:
  observe(2)(webuj.transactionObservable) { tx =>
    println(format(tx))
  }

  // Display the first 2 pending transactions as they are submitted to the network, before they have been grouped into a block:
  observe(2)(webuj.pendingTransactionObservable) { tx =>
    println(format(tx))
  }
  ...
  var count = 0
  webuj.catchUpToLatestAndSubscribeToNewBlocksObservable(EARLIEST, false).subscribe { hucBlock =>
    val block = hucBlock.getBlock
    if (block.getTimestamp.longValue<now) {
      count = count + 1
      print(s"\rSkipped $count blocks. ")
//      println(s"Skipping ${ block.getNumber }: ${ block.javaTime }")
    } else println(format(hucBlock))
  }
  ...
  // See https://github.com/happyuc-project/wiki/wiki/JSON-RPC#huc_newfilter
  val hucFilter: request.HucFilter =
    new request
      .HucFilter(EARLIEST, LATEST, contractAddress)
  ...
  webuj.hucLogObservable(hucFilter).subscribe { log =>
    println(format(log))
  }


  protected def format(hucBlock: response.HucBlock): String = {
    val block = hucBlock.getBlock
    s"""ETH block:
